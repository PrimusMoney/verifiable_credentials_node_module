...
class ReactNativeLoad {
	...
	init(callback) {
		...
		try {
			...
			var primus_react_client_wallet = this.node_module.primus_react_client_wallet;
			
			if (primus_react_client_wallet.initialized === false) {
				console.log('WARNING: @primusmoney/react_client_wallet should be initialized before initializing @primusmoney/verifiable_credentials');
			}
...
/* 			rootscriptloader.registerEventListener('@primusmoney/on_verifiable_credentials_module_ready', function(eventname) {
			...
			var xtrawebscriptloader = modulescriptloader.getChildLoader('@primusmoney/reactpwa');
			
			require('./loaders/react-pwa-load.js');
